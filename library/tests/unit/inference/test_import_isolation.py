...
class TestTorchFreeImports:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                "data.constants",
                "import sys; from physicalai.data.constants import "
                "ACTION, IMAGES, STATE; assert 'torch' not in sys.modules",
                id="data_constants",
            ),
            pytest.param(
                "inference.adapters.base",
                "import sys; from physicalai.inference.adapters.base import "
                "RuntimeAdapter; assert 'torch' not in sys.modules",
                id="adapter_base",
            ),
            pytest.param(
                "inference.adapters.onnx",
                "import sys; from physicalai.inference.adapters.onnx import "
                "ONNXAdapter; assert 'torch' not in sys.modules",
                id="onnx_adapter",
            ),
            pytest.param(
                "inference.adapters.openvino",
                "import sys; from physicalai.inference.adapters.openvino import "
                "OpenVINOAdapter; assert 'torch' not in sys.modules",
                id="openvino_adapter",
            ),
        ],
    )
    def test_single_module_no_torch(self, description: str, code: str) -> None:  # noqa: PLR6301
        ...
        assert result.returncode == 0, f"Importing {description} pulled in torch.\nstderr: {result.stderr}"  # noqa: S101

    def test_adapters_init_no_torch(self) -> None:  # noqa: PLR6301
    ...
    def test_inference_model_no_torch(self) -> None:  # noqa: PLR6301
...
class TestBackwardCompatibility:
    ...
    def test_backward_compat_export_backend(self) -> None:  # noqa: PLR6301
    ...
    def test_backward_compat_constants(self) -> None:  # noqa: PLR6301
    ...
    def test_backward_compat_adapters(self) -> None:  # noqa: PLR6301
