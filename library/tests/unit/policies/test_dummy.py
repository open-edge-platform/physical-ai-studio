...
class TestDummyPolicyValidation:
    ...
    def test_validation_step_accepts_gym_observation(self):
        """Test that validation_step accepts GymObservation."""
        ...
        result = policy.validation_step(gym_obs, batch_idx=0)

        # Should return a dict of metrics
        assert isinstance(result, dict)
        assert all(isinstance(v, (int, float, torch.Tensor)) for v in result.values())

    def test_test_step_accepts_gym_observation(self):
        """Test that test_step accepts GymObservation."""
        from getiaction.data.observation import GymObservation
        from getiaction.gyms import PushTGym
        from getiaction.policies.dummy import Dummy, DummyConfig
        ...
        # This should not raise TypeError
        result = policy.test_step(gym_obs, batch_idx=0)
    ...
    def test_validation_metrics_have_correct_keys(self):
        ...
        metrics = policy.validation_step(gym_obs, batch_idx=0)
    ...
    def test_test_metrics_use_test_prefix(self):
        ...
        metrics = policy.test_step(gym_obs, batch_idx=0)
