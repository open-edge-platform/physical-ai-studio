...
# ruff: noqa: FBT001, FBT002, PLR0914, PLR0915, PLR6104
# ^^^^ Disabled because this module mirrors the V-JEPA / timm module signatures the published
# checkpoints were trained with: boolean positional flags and wide attention forwards.

"""Action-conditioned V-JEPA world model for VLA-JEPA.

Ported from LeRobot's ``lerobot.policies.vla_jepa.world_model``. The predictor consumes
V-JEPA2 frame embeddings plus Qwen action tokens and predicts the next temporal position's
embeddings; the resulting L1 loss is an auxiliary training signal only and is not used at
inference time.
"""
...
def build_action_block_causal_attention_mask(
    ...
    """Build the block-causal attention mask over conditioning and patch tokens.

    Args:
        num_frames: Number of temporal positions.
        grid_height: Patch-grid height.
        grid_width: Patch-grid width.
        add_tokens: Conditioning tokens prepended to every frame.

    Returns:
        Boolean mask of shape ``[num_tokens, num_tokens]``, True where attention is allowed.
    """
...
def rotate_queries_or_keys(x: torch.Tensor, pos: torch.Tensor) -> torch.Tensor:
    """Apply rotary position encoding along one axis.

    Args:
        x: Queries or keys of shape ``[B, heads, tokens, dim]``.
        pos: Positions to rotate by.

    Returns:
        The rotated tensor.

    Raises:
        ValueError: If the embedding dimension is odd.
    """
    _, _, _, dim = x.size()
    if dim % 2 != 0:
        msg = "Embedding dimension must be even for rotary position encoding."
...
class DropPath(nn.Module):
    """Stochastic depth: drops whole residual branches during training."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Drop the residual branch for a random subset of the batch.

        Args:
            x: Residual branch output.

        Returns:
            The rescaled input, or the input unchanged in eval mode.
        """
...
class MLP(nn.Module):
    """Two-layer feed-forward block used inside the predictor."""
...
class ACRoPEAttention(nn.Module):
    """Action-conditioned attention with axial rotary position encoding."""
    ...
    def __init__(
        ...
        Args:
            ...
            qk_scale: Optional override of the attention scale.
            ...
            grid_size: Reference patch-grid size the rotary encoding is calibrated for.
        """
        super().__init__()
    ...
    def separate_positions(
        ...
        """Split flat token ids into frame, height and width positions.

        Args:
            ids: Flat token indices.
            height: Patch-grid height.
            width: Patch-grid width.

        Returns:
            Tuple of (frame, height, width) positions as floats.
        """
        frame_ids = self._get_frame_pos(ids, height, width)
    ...
    def forward(
        ...
        """Attend over the conditioning and patch tokens of every frame.

        Args:
            x: Input tokens of shape ``[B, tokens, channels]``.
            mask: Optional explicit token positions; defaults to a dense grid.
            attn_mask: Optional additive/boolean attention mask.
            num_frames: Number of temporal positions in `x`.
            grid_height: Patch-grid height.
            grid_width: Patch-grid width.
            action_tokens: Conditioning tokens prepended to every frame.

        Returns:
            The attended tokens, same shape as `x`.

        Raises:
            ValueError: If the frame or grid geometry is not given.
        """
        batch_size, num_tokens, channels = x.size()
        if num_frames is None or grid_height is None or grid_width is None:
            msg = "num_frames, grid_height and grid_width are required."
            raise ValueError(msg)
        ...
        if action_tokens > 0:
            ...
            for idx in range(action_tokens):
                ...
                qd = rotate_queries_or_keys(
                    q[..., : self.d_dim],
                    pos=torch.arange(num_frames, device=x.device),
                )
        ...
        if action_tokens > 0:
            ...
            q = merge(q, action_q)  # pyrefly: ignore[unbound-name]
            k = merge(k, action_k)  # pyrefly: ignore[unbound-name]
            v = merge(v, action_v)  # pyrefly: ignore[unbound-name]
...
class ACBlock(nn.Module):
    """Pre-norm transformer block wrapping :class:`ACRoPEAttention` and an MLP."""
    ...
    def __init__(
        ...
        """Initialize the predictor block.
        ...
        Args:
            ...
            qkv_bias: Whether the qkv projection has a bias.
            qk_scale: Optional override of the attention scale.
            drop: Dropout probability on projections and the MLP.
            attn_drop: Attention dropout probability.
            drop_path: Stochastic-depth probability.
            norm_layer: Normalization factory taking the token width.
            use_sdpa: Whether to use scaled dot-product attention.
            is_causal: Whether attention is causal.
            grid_size: Reference patch-grid size for the rotary encoding.
            use_rope: Must be True; the predictor is defined with rotary attention.

        Raises:
            ValueError: If `use_rope` is False.
        """
        super().__init__()
        self.norm1 = norm_layer(dim)
        if not use_rope:
            msg = "JEVLA1 world predictor uses AC RoPE attention."
            raise ValueError(msg)
    ...
    def forward(
        ...
        """Run attention and the feed-forward block with residual connections.
...
class ActionConditionedVideoPredictor(nn.Module):
    ...
    def __init__(
        ...
        Args:
            num_frames: Number of temporal positions after tubelet compression.
            img_size: Spatial size ``(height, width)`` the encoder tokenizes.
            patch_size: Encoder patch size.
            tubelet_size: Temporal tubelet size.
            embed_dim: Width of the incoming frame embeddings (encoder hidden x views).
            action_embed_dim: Width of the incoming action tokens.
            predictor_embed_dim: Internal predictor width.
            depth: Number of predictor blocks.
            num_heads: Attention heads per block.
            mlp_ratio: Feed-forward expansion ratio.
            num_action_tokens_per_step: Action tokens conditioning each temporal position.
            dropout: Dropout probability.
            use_extrinsics: Whether camera extrinsics are appended as an extra token.
        """
        super().__init__()
    ...
    def norm(self) -> nn.LayerNorm:
        """The output normalization layer."""
        return self.predictor_norm

    @property
    def proj(self) -> nn.Linear:
        """The output projection back to the encoder's embedding width."""
    ...
    def forward(
        ...
        """Predict the next temporal position's frame embeddings.

        Args:
            frame_tokens: Context frame embeddings ``[B, T*H*W, embed_dim]``.
            action_tokens: Action tokens ``[B, T*A, action_embed_dim]``.
            extrinsics: Camera extrinsics, required when `use_extrinsics` is set.

        Returns:
            Predicted frame embeddings, same shape as `frame_tokens`.

        Raises:
            ValueError: If `use_extrinsics` is set but no extrinsics are given.
        """
        ...
        if self.use_extrinsics:
            if extrinsics is None or self.extrinsics_encoder is None:
                msg = "extrinsics are required when use_extrinsics=True."
                raise ValueError(msg)
            cond_tokens += 1
