...
from .xvla import XVLA, XVLAConfig, XVLAModel

# Grouped by policy family rather than sorted: an isort-style sort would file the
# all-caps names (ACT, XVLA) ahead of every class name and break the grouping.
__all__ = [  # noqa: RUF022
    # ACT
    "ACT",
    "ACTConfig",
...
def get_policy(policy_name: str, *, source: str = "physicalai", **kwargs) -> Policy:  # noqa: ANN003
    ...
    Args:
        policy_name: Name of the policy to create. Supported values depend on source:
            - physicalai: "act", "dummy", "groot", "pi0", "pi05", "smolvla", "xvla"
...
def get_physicalai_policy_class(policy_name: str) -> type[Policy]:
    ...
    if policy_name == "xvla":
        return XVLA
    msg = f"Unknown physicalai policy: {policy_name}. Supported policies: act, dummy, groot, pi0, pi05, smolvla, xvla"
    raise ValueError(msg)
