...
"""Pi0 (Physical Intelligence) policy wrapper.

This module provides a Lightning-compatible wrapper around LeRobot's Pi0 implementation,
Physical Intelligence's foundation model for robot manipulation.

## Quick Start

Train Pi0 using the provided YAML config:
...
getiaction fit --config configs/lerobot/pi0.yaml
...
Pi0 fully supports Intel XPU (Data Center GPUs, Arc GPUs) because it uses
...
Pi0 **cannot be exported to ONNX/OpenVINO/TorchExportIR** because:
...
```python
from getiaction.policies.lerobot import Pi0
...
- `configs/lerobot/pi0.yaml`: Default training configuration
- [LeRobot Pi0](https://github.com/huggingface/lerobot): Upstream implementation
...
class Pi0(LeRobotFromConfig, Policy):
    """Pi0 (Physical Intelligence) policy from LeRobot.

    PyTorch Lightning wrapper around LeRobot's Pi0 implementation, Physical
    Intelligence's vision-language-action foundation model for robot manipulation.

    Pi0 uses PaliGemma (a multimodal model combining SigLIP vision encoder with
    Gemma language model) combined with an action expert network. It uses flow
    matching for action prediction with iterative denoising at inference time.

    ## Device Support

    Pi0 supports CUDA, Intel XPU, and CPU because it uses PyTorch's standard
    "eager" attention implementation. Unlike Groot which requires Flash Attention,
    Pi0 can run on any PyTorch-supported accelerator.
    ...
    **Inference**
    ...
    policy = Pi0.load_from_checkpoint("path/to/checkpoint.ckpt")
    policy.eval()
    ...
    def __init__(  # noqa: PLR0913
        ...
        **kwargs: Any,  # noqa: ANN401
        ...
        Args:
            ...
            n_obs_steps: Number of observation steps (typically 1 for Pi0).
        ...
        if not LEROBOT_AVAILABLE:
            msg = (
                "Pi0 requires LeRobot framework.\n\n"
                "Install with:\n"
                "    pip install lerobot\n\n"
                "Note: Pi0 uses standard PyTorch attention (no Flash Attention required),\n"
    ...
    def lerobot_policy(self) -> _LeRobotPI0Policy:
        ...
        Returns:
            The initialized LeRobot Pi0 policy.
    ...
    def forward(self, batch: Observation | dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        Note:
            Pi0 does NOT support export to ONNX/OpenVINO due to:
