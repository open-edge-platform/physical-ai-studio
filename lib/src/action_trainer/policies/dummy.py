...
class DummyModel(nn.Module):
    ...
    def forward(self, batch: dict[str, torch.Tensor]) -> tuple[torch.Tensor, dict]:
        """In train mode: return dummy loss; in eval: delegate to predict_action_chunk."""
        if self.training:
            # produce random predictions and compute dummy loss
            pred = self.predict_action_chunk(batch)
