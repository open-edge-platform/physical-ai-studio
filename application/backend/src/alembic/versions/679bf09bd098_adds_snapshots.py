...
def upgrade() -> None:
    ...
    snapshots_table = op.create_table("snapshots",
    ...
    conn = op.get_bind()
    model_rows = conn.execute(text("SELECT * FROM models INNER JOIN (SELECT path as dataset_path, id as d_id FROM datasets) on d_id = models.dataset_id;")).fetchall()
    snapshots = []
    models = []
