...
def _payload(*, remote_job_id: str | None = None) -> TrainJobPayload:
...
class TestReattachOrphans:
    ...
    async def test_remote_running_job_with_remote_id_is_requeued(self):
        ...
        job = _job(_payload(remote_job_id="trainer-job-1"))
    ...
    async def test_local_mode_always_fails_orphans(self):
        ...
        job = _job(_payload(remote_job_id="trainer-job-2"))
    ...
    def test_reattachable_remote_job_id_reads_dict_payload(self):
        ...
        job = MagicMock()
        job.payload = {"remote_job_id": "trainer-job-3"}
        assert TrainingService._reattachable_remote_job_id(job) == "trainer-job-3"
