...
4. No arbitrary `class_path` import. Validate against an allowlist (e.g., `physicalai.`, `lerobot.`, `lightning.`, `torch.nn.` or `torch.optim.`) before passing strings to `importlib.import_module()` or `getattr(module, name)`. Each prefix must end with a trailing dot — `"torch.nn."` not `"torch.nn"` — to block prefix-collision bypasses (e.g., `torch.nn_evil`). Applies to `config/instantiate.py`, `cli/_policy.py:load_policy()`, `trainer.callbacks`, and `manifest.json` inference adapters.
